...
public class UserRepository {
    ...
    public int updateProfileAllFields(
            int userId,
            String nickname,
            String name,
            String email,
            String phone,
            String address,
            String passwordOrNull // null/blank이면 비번은 건드리지 않음
    ) {
        StringBuilder sql = new StringBuilder(
                "UPDATE users SET nickname = ?, name = ?, email = ?, phone = ?, address = ?"
        );

        // 비밀번호가 들어오면 비밀번호도 수정
        boolean changePw = (passwordOrNull != null && !passwordOrNull.trim().isEmpty());
        if (changePw) {
            sql.append(", password = ?");
        }
        ...
        return jdbc.update(con -> {
            ...
            if (changePw) {
                ps.setString(i++, passwordOrNull); // 실제 서비스에선 반드시 해시하세요!
            }
            ps.setInt(i++, userId);
            return ps;
        });
