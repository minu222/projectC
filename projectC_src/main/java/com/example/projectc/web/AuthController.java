...
public class AuthController {
    ...
    // =========================
    // 아이디(닉네임) 중복 체크
    // GET /api/auth/check-userid?userid=woong
    // =========================
    ...
    public ResponseEntity<?> checkUserId(@RequestParam("userid") String userid) {
        ...
        if (!uid.matches("^[A-Za-z0-9_]{4,20}$")) {
            return ResponseEntity.ok(Map.of("ok", true, "available", false, "msg", "아이디는 영문/숫자/밑줄 4~20자"));
        }
    ...
    private ResponseEntity<?> doSignup(SignupForm f) {
        // userid(닉네임) 검증/중복
        if (isBlank(f.getUserid()) || !f.getUserid().trim().matches("^[A-Za-z0-9_]{4,20}$")) {
            return ResponseEntity.badRequest().body(Map.of("ok", false, "msg", "아이디는 영문/숫자/밑줄 4~20자로 입력하세요."));
        }
    ...
    private ResponseEntity<?> doLoginByNickname(String nickname, String password, HttpSession session) {
        ...
        // 닉네임 형식 검증(이메일 금지)
        if (nickname.contains("@") || !nickname.matches("^[A-Za-z0-9_]{4,20}$")) {
